...
class BlackjackEnv(gym.Env):
    ...
    def tuple_to_index(self, state_tuple):
        return int(np.ravel_multi_index(state_tuple, self.dims))

    def index_to_tuple(self, state_index):
        return tuple(int(x) for x in np.unravel_index(state_index, self.dims))

    def reset(self, seed=None, options=None):
        state_tuple, info = self.env.reset(seed=seed, options=options)
        state_index = self.tuple_to_index(state_tuple)
        return state_index, info

    def step(self, action):
        state_tuple, reward, terminated, truncated, info = self.env.step(action)
        state_index = self.tuple_to_index(state_tuple)
        return state_index, reward, terminated, truncated, info
    
    def render_state(self, state, **kwargs):
        state_tuple = self.index_to_tuple(state)
        return f'Player\'s sum: {state_tuple[0]}\nDealer\'s card: {state_tuple[1]}\nUsable ace: {"yes" if state_tuple[2] == 1 else "no"}'
